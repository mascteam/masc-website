...
const TargetCursor = ({
  ...
  useEffect(() => {
     ...
     * IMPORTANT:
     *
     * This checks whether the cursor is STILL physically over
     * the active target.
     *
     * mouseleave alone is not reliable when the target itself
     * moves underneath/away from the cursor.
     */
    const checkTargetUnderCursor = () => {
      if (
        !activeTarget ||
        !cursorRef.current ||
        !currentLeaveHandler
    ...
    const tickerFn = () => {
       ...
       * NEW:
       *
       * Check every frame whether the moving target is still
       * underneath the cursor.
       */
      checkTargetUnderCursor();

      // The leave handler may have run above.
      if (
        !activeTarget ||
        !targetCornerPositionsRef.current
      ) {
      ...
      corners.forEach((corner, i) => {
        ...
        const finalX =
          currentX +
          (targetX - currentX) * strength;

        const finalY =
          currentY +
          (targetY - currentY) * strength;
    ...
    /*
     * Scroll can also move a target, so keep this check.
     */
    const scrollHandler = () => {
      checkTargetUnderCursor();
    };
