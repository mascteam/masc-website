...
const HeroToast = () => {
  ...
  return (
    event && (
      <div onClick={()=> router.push(`/events/${event.slug}`)} className="hidden cursor-target cursor-pointer md:flex flex-row justify-around items-center border-2 border-black rounded-sm w-[30%] h-1/2 p-x">
        <img src={event.banner} className="h-full w-[35%] md:w=[30%] object-cover p-1" />
